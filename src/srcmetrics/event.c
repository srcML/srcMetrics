...
static void event_startDocument(struct srcsax_context* context) {
    ...
    *lastEventOfProcInfo                = NULL;
    ...
    unless (
        (isValid_chunk(strings) || isValid_chunk((strings = constructEmpty_chunk(BUFSIZ))))                 &&
        ...
        (functionStack || (functionStack = calloc(functionStack_cap, sizeof(Function))))
...
static void event_startElement(
    ...
    if (str_eq_const(localname, "function")) {
        ...
        functions[functions_count++] = *(currentFunction = functionStack + functionStack_size++) = (Function){ "Unknown Function", currentUnit->name, NULL };
...
static void event_endElement(
    ...
    } else if (function_read_state == 5 && (!currentFunction || str_eq_const(currentFunction->name, "Unknown Function"))) {
        fputs("PARSE_ERROR\n", stderr);
        exit(EXIT_FAILURE);
    }
    ...
    unless (closeFn) return;
    switch (function_read_state) {
        case 5:
            currentFunction = functionStack + functionStack_size - 1;
        case 0:
            currentFunction = NULL;
    }
}
